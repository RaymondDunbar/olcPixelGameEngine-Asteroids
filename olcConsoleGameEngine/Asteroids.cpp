...
protected:
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		if (m_keys[VK_SPACE].bReleased)
			vecBullets.push_back({ 0, player.x, player.y, 100.0f * sinf(player.angle), -100.0f * cosf(player.angle), 100.0f });
			...
			for (auto& a : vecAsteroids)
			{
				...
				if (IsPointInsideCircle(a.x, a.y, a.nSize, b.x, b.y))
				{
					...
					if (a.nSize > 4)
					{
						...
						newAsteroids.push_back({ (int)a.nSize >> 1 ,a.x, a.y, a.dx * sinf(angle1) * 2.0f, a.dy * cosf(angle1)*2.0f, 0.0f });
						newAsteroids.push_back({ (int)a.nSize >> 1 ,a.x, a.y, a.dx * sinf(angle2) * 2.0f, a.dy * cosf(angle2)*2.0f, 0.0f });
					}
		...
		if (vecAsteroids.empty()) // If no asteroids, level complete! :) - you win MORE asteroids!
		{
			...
			case 2:
				...
				vecAsteroids.push_back({ (int)16, 30.0f * sinf(player.angle + 3.14159f / 2.0f) + player.x,
													  30.0f * cosf(player.angle + 3.14159f / 2.0f) + player.y,
													  25.0f * sinf(-player.angle + 90), 25.0f * cosf(-player.angle + 90), 0.0f });
				break;

			case 3:
				vecAsteroids.push_back({ (int)16, 30.0f * sinf(player.angle - 3.14159f / 2.0f) + player.x,
											  30.0f * cosf(player.angle - 3.14159f / 2.0f) + player.y,
											  35.0f * sinf(player.angle), 35.0f * cosf(player.angle), 0.0f });
				...
				vecAsteroids.push_back({ (int)16, 30.0f * sinf(player.angle - 3.14159f / 2.0f) + player.x,
											  30.0f * cosf(player.angle - 3.14159f / 2.0f) + player.y,
											  10.0f * sinf(player.angle + 90), 10.0f * cosf(player.angle + 90), 0.0f });

				vecAsteroids.push_back({ (int)16, 30.0f * sinf(player.angle + 3.14159f / 2.0f) + player.x,
												  30.0f * cosf(player.angle + 3.14159f / 2.0f) + player.y,
												  35.0f * sinf(-player.angle + 90), 35.0f * cosf(-player.angle + 90), 0.0f });
				break;
			case 4:
				vecAsteroids.push_back({ (int)16, 30.0f * sinf(player.angle + 3.14159f / 2.0f) + player.x,
													  30.0f * cosf(player.angle + 3.14159f / 2.0f) + player.y,
													  50.0f * sinf(-player.angle), 50.0f * cosf(-player.angle), 0.0f });

				vecAsteroids.push_back({ (int)16, 30.0f * sinf(player.angle + 3.14159f / 2.0f) + player.x,
													  30.0f * cosf(player.angle + 3.14159f / 2.0f) + player.y,
													  55.0f * sinf(-player.angle), 55.0f * cosf(-player.angle), 0.0f });

				vecAsteroids.push_back({ (int)16, 30.0f * sinf(player.angle + 3.14159f / 2.0f) + player.x,
													  30.0f * cosf(player.angle + 3.14159f / 2.0f) + player.y,
													  55.0f * sinf(-player.angle + 90), 55.0f * cosf(-player.angle + 90), 0.0f });

				vecAsteroids.push_back({ (int)16, 30.0f * sinf(player.angle + 3.14159f / 2.0f) + player.x,
													  30.0f * cosf(player.angle + 3.14159f / 2.0f) + player.y,
													  60.0f * sinf(-player.angle - 90), 60.0f * cosf(-player.angle - 90), 0.0f });

				vecAsteroids.push_back({ (int)16, 30.0f * sinf(player.angle + 3.14159f / 2.0f) + player.x,
													  30.0f * cosf(player.angle + 3.14159f / 2.0f) + player.y,
													  15.0f * sinf(-player.angle + 45), 15.0f * cosf(-player.angle + 45), 0.0f });
...
int main()
{
	...
	game.ConstructConsole(200, 120, 4, 4);
	game.Start();
	return 0;
}